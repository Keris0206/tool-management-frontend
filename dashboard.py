...
# 1️⃣ API base URL
# -----------------------------
#API_URL = "https://tool-management-backend-lscs.onrender.com"
import os
    ...
    if not tool_id_input or not operator_name:
        ...
        try:
            response = requests.post(
                ...
                data={"operator": operator_name, "action": action}
            )
            if response.status_code == 200:
...
try:
    # Fetch tool history from backend
    ...
    if history_response.status_code == 200:
        ...
        if not history_df.empty:
            ...
            # Convert timestamp fields to datetime (UTC first), then to Singapore time
            history_df["checkout_time"] = pd.to_datetime(history_df["checkout_time"], errors='coerce', utc=True).dt.tz_convert(sg_tz)
            history_df["return_time"] = pd.to_datetime(history_df["return_time"], errors='coerce', utc=True).dt.tz_convert(sg_tz)
...
# -----------------------------
# 8️⃣ Optional: filter by status
# -----------------------------
st.subheader("🔍 Filter Tools")
status_filter = st.selectbox("Filter by Status", ["All", "Available", "In Use", "Maintenance"])
...
if status_filter != "All":
    filtered_df = tools_df[tools_df["status"] == status_filter]
>>>>>>> 276530adde1582aac3806dccf829adb99e7f3702
